...
public class MediationServerParameters {
    ...
    public void load(Map<String, String> map) throws MappingException {
        ...
        if (hashMap.isEmpty()) {
            zzakb.zzcu("No server options fields detected. To suppress this message either add a field with the @Parameter annotation, or override the load() method.");
        }
        ...
        for (Field field3 : hashMap.values()) {
            if (((Parameter) field3.getAnnotation(Parameter.class)).required()) {
                ...
                zzakb.zzcu(valueOf.length() != 0 ? "Required server option missing: ".concat(valueOf) : new String("Required server option missing: "));
                if (sb3.length() > 0) {
                    sb3.append(", ");
                }
